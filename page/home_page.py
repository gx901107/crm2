...
class HomePage(BasePage):
    ...
    save_locator = (By.CSS_SELECTOR, '#dialog-message > form > div:nth-child(4) > div > input.btn.btn-primary') #点击保存组件
    add_task_locator = (By.CSS_SELECTOR, '#calendar > div.c-event-grid > div.c-task-body > div.data-head > a > i') #快捷添加任务
    wukong_locator = (By.CSS_SELECTOR, 'body > div.navbar.navbar-inverse.navbar-fixed-top > div > div > a') #点击悟空首页
    add_schedule_locator = (By.CSS_SELECTOR, '#calendar > div.c-event-grid > div.c-event-body > div.data-head > a > i') #快捷添加日程
    schedule_name_locator = (By.CSS_SELECTOR, '#subject') #输入日程主题名
    save_schedule_locator = (By.CSS_SELECTOR, 'body > div.container > div.row > div > form > table > tfoot > tr > td:nth-child(2) > input:nth-child(1)') #保存新日程
    affiche_list_locator = (By.CSS_SELECTOR, '#widgets > div > div:nth-child(6) > div > div.dash-title > a') #切换到公告列表
    head_portrait_locator = (By.CSS_SELECTOR, 'body > div.navbar.navbar-inverse.navbar-fixed-top > div > div > div.nav-collapse.collapse > ul.nav.pull-right > li:nth-child(6) > a') #点击头像
    my_data_locator = (By.CSS_SELECTOR, 'body > div.navbar.navbar-inverse.navbar-fixed-top > div > div > div.nav-collapse.collapse > ul.nav.pull-right > li.dropdown.open > ul > li:nth-child(3) > a') #个人资料
    email_locator = (By.CSS_SELECTOR, 'body > div.container > div.row > div:nth-child(2) > form > table > tbody > tr:nth-child(11) > td:nth-child(2) > input') #邮箱号
    phone_locator = (By.CSS_SELECTOR, 'body > div.container > div.row > div:nth-child(2) > form > table > tbody > tr:nth-child(12) > td:nth-child(2) > input') #手机号
    save_data_locator = (By.XPATH, '/html/body/div[5]/div[2]/div[2]/form/table/tfoot/tr/td[2]/input[1]') #保存修改的个人资料
    organizational_structure_locator = (By.CSS_SELECTOR, 'body > div.navbar.navbar-inverse.navbar-fixed-top > div > div > div.nav-collapse.collapse > ul.nav.pull-right > li.dropdown.open > ul > li:nth-child(4) > a') #组织架构
    operation_log_locator = (By.CSS_SELECTOR, 'body > div.navbar.navbar-inverse.navbar-fixed-top > div > div > div.nav-collapse.collapse > ul.nav.pull-right > li.dropdown.open > ul > li:nth-child(7) > a') #操作日志
